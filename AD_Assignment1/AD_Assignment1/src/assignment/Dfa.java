...
* */
    ...
    /*
    * this method will check if the string is part of the
    * by recursively going to every transition of that string checking if
    * the last character is in the accept state or not
    * */
    public  boolean isPartOfTheLanguage(int startIndex,int tempState,String testData){
        if(startIndex==testData.length()){
            if(transTable[tempState][0]==ACCEPT)
                return true;
            else
                return false;
        }
        int charOfS = testData.charAt(startIndex);

        int nextState = transTable[tempState][charOfS+1-SIGMA_LOWER];
        return isPartOfTheLanguage(startIndex++,nextState,testData);
   }
}